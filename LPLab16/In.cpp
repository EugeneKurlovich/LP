...
namespace In
{
	IN getin(std::ofstream *stream, wchar_t Infile[]) 
	{
		...
		unsigned char readSymbol = IN_CODE_NULL;  // „тобы русские символы инициализировались положительным числом
		...
		while ((readSymbol = txtfile.get()) && (!txtfile.eof()))
		{
			...
			case IN::STAB:
				...
				case IN::T:
				{
					if (StrIn.code[StrIn.text[StrIn.size - 2]] == IN::SEP && StrIn.code[StrIn.text[StrIn.size - 1]] == IN::SPACE)
						StrIn.text[StrIn.size - 1] = readSymbol;
					else 
						StrIn.text[StrIn.size++] = readSymbol;
					break;
